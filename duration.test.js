...
describe("duration", () => {
  ...
  it("check parsing miliseconds from date", () => {
      ...
      expect(result).toEqual({ day: 21, hour: 0, min: 0, sec: 0, ms: 0, year: 0, month: 1 });
      expect(result2).toEqual({ day: 20, hour: 5, min: 58, sec: 50, ms: 0, year: 0, month: 0 });
      expect(result3).toEqual({ day: 20, hour: 5, min: 58, sec: 50, ms: 0, year: 1, month: 0 });
    });
});
